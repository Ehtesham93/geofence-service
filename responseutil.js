// API Response helpers: enforce a consistent envelope across all endpoints
...
export function ApiResponseFabErr(req, res, faberrI) {
    return APIResponseInternalErr(req, res, faberrI.errcode, faberrI.errdata, faberrI.errmsg);
}

export function APIResponseError(req, res, statuscode, errcode, errdata, msg) {
    return res.status(statuscode).send(apiresponse(apiresponseerror(errcode, errdata), null, msg));
}
